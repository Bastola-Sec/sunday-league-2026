                                    ...
                                    <div className="p-4 rounded-2xl bg-[#0d1a28] border border-[#4C787E]/40 space-y-4">
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-[11px] text-gray-400">
                                          Sunday League Half Duration:{' '}
                                          <strong className="text-white">{halfDuration} Mins</strong>
                                        </span>
                                      </div>

                                      {/* State Flow Action Buttons */}
                                      <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
                                        {/* Kickoff 1st Half */}
                                        <button
                                          onClick={handleTriggerKickoff1stHalf}
                                          disabled={editingMatch.status === '1st_half' || editingMatch.status === '2nd_half' || editingMatch.status === 'ended'}
                                          className={`py-3 px-3 rounded-xl font-black text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md ${editingMatch.status === 'scheduled' || !editingMatch.status
                                        ...
                                        <button
                                          ...
                                          disabled={editingMatch.status !== '1st_half'}
                                          className={`py-3 px-3 rounded-xl font-black text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md ${editingMatch.status === '1st_half'
                                        ...
                                        {/* Kickoff 2nd Half */}
                                        <button
                                          onClick={handleTriggerKickoff2ndHalf}
                                          disabled={editingMatch.status !== 'halftime'}
                                          className={`py-3 px-3 rounded-xl font-black text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md ${editingMatch.status === 'halftime'
                                            ? 'bg-gradient-to-r from-emerald-500 to-teal-600 text-slate-950 hover:brightness-110 ring-2 ring-emerald-400'
                                            : 'bg-slate-800 text-gray-500 cursor-not-allowed opacity-60'
                                            }`}
                                        >
                                        ...
                                        {/* End Match (Full Time) */}
                                        <button
                                          onClick={handleTriggerEndMatch}
                                          disabled={editingMatch.status !== '2nd_half'}
                                          className={`py-3 px-3 rounded-xl font-black text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md ${editingMatch.status === '2nd_half'
